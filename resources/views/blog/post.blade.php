...
@section('content')
    <main id="site-main" class="site-main max-w-5xl mx-auto md:mt-10 px-5 flex flex-wrap justify-center rounded-3xl">
        ...
        <div class="post-page mx-auto md:my-8 my-5 w-full flex flex-col justify-around">
            <div class="breadcrubs inline-flex text-base items-center">
                ...
                <a class="mr-1 border-b border-primary hover:border-primary dark:hover:border-primary-darker border-opacity-30"
                    href="{{ route('getPostsByCategory', $post->category['slug']) }}">
                    {{ $post->category['title'] }}
                </a>
            ...
            <article class="post mx-auto mt-8 w-full">
                <header class="post-header">
                    <h1 class="post-title font-bold text-3xl sm:text-4xl text-black dark:text-light-gray">
                        {{ $post->title }}</h1>
                </header>
                    ...
                    @if ($post->tags->isNotEmpty())
                        <span class="post-tag text-xs inline-flex mx-4">
                            ...
                            @foreach ($post->tags as $tag)
                                <a class="text-xs mr-1 border-b border-primary hover:border-primary dark:hover:border-primary-darker border-opacity-30"
                                    href="{{ route('getPostsByTag', $tag['slug']) }}">{{ $tag->name }}</a>
                            @endforeach
                        </span>
                    @endif
