                ...
                <article class="posts mb-5 flex flex-wrap md:flex-nowrap border-b-2 border-gray border-opacity-10">
                    ...
                    <div
                        ...
                        @if ($post->tags->isNotEmpty())
                            <span class="post-tag inline-flex">
                                ...
                                @foreach ($post->tags as $tag)
                                    <a class="text-primary dark:text-primary-darker hover:text-primary dark:hover:text-primary-darker text-xs mr-1 border-b border-primary hover:border-primary dark:hover:border-primary-darker border-opacity-30 dark:border-opacity-30"
                        ...
                        <span class="text-xs inline-flex">
                            ...
                            </svg><span>{{ $post->comments->whereNotNull('online')->count() }}</span>
                        </span>

                        @if (Auth::check())
                            @if (Auth::user()->isAdmin)
                                <a class="text-xs inline-flex border-0" href="{{ route('posts.edit', $post) }}">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1" fill="none"
                                        viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <div class="post-block w-full md:flex-grow">
                        <h2 class="post-title font-bold font-header text-3xl sm:text-4xl">
