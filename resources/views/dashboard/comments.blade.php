...
@section('content')
<h1 class="text-2xl font-bold text-black pb-6">Комментарии</h1>
        ...
        <table class="min-w-full bg-white">
            <thead class="bg-gray-500 text-white">
                <tr>
                    ...
                    <th class="text-center py-3 px-4 uppercase font-semibold text-sm">Пост</th>
                    <th class="text-center py-3 px-4 uppercase font-semibold text-sm">Коммент</th>
                    <th class="text-center py-3 px-4 uppercase font-semibold text-sm">Автор</th>
                    <th class="text-center py-3 px-4 uppercase font-semibold text-sm">Вебсайт</th>
                    <th class="text-center py-3 px-4 uppercase font-semibold text-sm">Статус</th>
            ...
            <tbody class="text-gray-700">
                ...
                <tr @if ($loop->even) class="bg-gray-200" @endif>
                    ...
                    <td class="text-center py-3 px-4">
                        <form id="online-form{{ $comment->id}}" action="{{ route('onlineComment', $comment) }}" method="POST">
                            @csrf
                            @method('PATCH')
                            ...
                            <label class="switch">
                                <input type="checkbox"
                                name="online"
                                @if ($comment->online) checked @endif
                                onclick="document.getElementById('online-form{{ $comment->id}}').submit();" value="1">
                                ...
                                <div class="overflow-auto" style="background-color: rgba(0,0,0,0.5)" x-show="showModal" :class="{ 'absolute inset-0 z-10 flex items-center justify-center': showModal }">
                                    ...
                                    <div class="bg-white w-11/12 md:max-w-md mx-auto rounded shadow-lg py-4 text-left px-6" x-show="showModal" @click.away="showModal = false" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 scale-90" x-transition:enter-end="opacity-100 scale-100" x-transition:leave="ease-in duration-300" x-transition:leave-start="opacity-100 scale-100" x-transition:leave-end="opacity-0 scale-90">
                                        ...
                                        <div class="flex justify-between items-center pb-3">
                                            <p class="text-2xl font-bold">Удалить коммент?</p>
