        ...
        <div class="bg-white shadow-md rounded-lg overflow-auto">
            <table class="min-w-full bg-white">
                ...
                <tr>
                    ...
                    <th class="text-center py-3 px-4 uppercase font-semibold text-sm">Дата
                    </td>
                    <th class="text-center py-3 px-4 uppercase font-semibold text-sm">Статус
                    </td>
                    <th class="text-center py-3 px-4 uppercase font-semibold text-sm">Правка
                    </td>
                    <th class="text-center py-3 px-4 uppercase font-semibold text-sm">Удалить
                    </td>
                </tr>
                </thead>
                ...
                @foreach ($posts as $post)
                    <tr @if ($loop->even) class="bg-gray-200" @endif>
                        ...
                        <td class="text-center py-3 px-4"><a class="hover:text-green-400"
                                                             href="{{ route('getPost', $post->slug) }}">{{ $post->title }}</a>
                        </td>
                        ...
                        <td class="text-center py-3 px-4">
                            @foreach ($post->tags as $tag)
                                <span
                                    class="inline-block rounded-md text-white bg-purple-500 px-2 py-1 text-xs font-bold mr-3">{{ $tag->name }}</span>
                            @endforeach
                        </td>
                        <td class="text-center py-3 px-4">
                        ...
                        <td class="text-center py-3 px-4">
                            <form id="online-form{{ $post->id }}" action="{{ route('online', $post) }}"
                                  method="POST">
                                @csrf
                                @method('PATCH')
                                ...
                                <label class="switch">
                                    <input type="checkbox" name="online"
                                           ...
                                           onclick="document.getElementById('online-form{{ $post->id }}').submit();"
                                           value="1">
                        ...
                        <td class="text-center py-3 px-4"><a href="{{ route('posts.edit', $post) }}"
                                                             class="text-xs px-3 py-1 bg-yellow-200 text-yellow-800 rounded-full">
                        ...
                        <td class="text-center py-3 px-4">
                            <form action="{{ route('posts.destroy', $post) }}" method="POST">
                                ...
                                <div x-data="{ 'showModal': false }" @keydown.escape="showModal = false" x-cloak>
                                    <button class="delete-btn text-xs px-3 py-1 bg-red-600 text-gray-50 rounded-full"
                                        ...
                                        <!--Footer-->
                                            <div class="flex justify-end pt-2">
                                                ...
                                                <button
                                                    ...
                                                    type="button" @click="showModal = false">Нет
                                                </button>
                                            </div>
                                        </div>
                                        <!--/Dialog -->
