    ...
    <div class="w-full mt-6" x-data="{ openTab: 1 }">
        ...
        <div class="bg-white p-6">
            <div id="" class="" x-show="openTab === 1">
                ...
                <form action="{{ route('settings.update', $site_name) }}" method="POST">
                    @csrf
                    @method('PATCH')
                    <div class="md:w-1/3 sm:mr-5 mb-5">
                        <label class="block text-sm text-gray-600" for="title">Название сайта</label>
                        <input class="w-full px-5 py-1 text-gray-700 bg-gray-200 rounded" id="option{{ $site_name->id}}" name="option" type="text" required="" placeholder="Название сайта"
                        value="{{ $site_name->option }}"
                        onfocusout="event.preventDefault(); document.getElementById('option{{ $site_name->id}}'); form.submit();">
                    </div>
                </form>

                <form action="{{ route('settings.update', $site_description) }}" method="POST">
                    @csrf
                    @method('PATCH')
                    <div class="md:w-1/3 sm:mr-5 mb-5">
                        <label class="block text-sm text-gray-600" for="title">Описание</label>
                        <input class="w-full px-5 py-1 text-gray-700 bg-gray-200 rounded" id="option{{ $site_description->id}}" name="option" type="text" required="" placeholder="Описание сайта"
                            value="{{ $site_description->option }}"
                            onfocusout="event.preventDefault(); document.getElementById('option{{ $site_description->id}}'); form.submit();">
                    </div>
                </form>

                <form action="{{ route('settings.update', $site_footer) }}" method="POST">
                    @csrf
                    @method('PATCH')
                    <div class="md:w-2/3 sm:mr-5 mb-5">
                        <label class="block text-sm text-gray-600" for="title">Футер</label>
                        <textarea class="w-full px-5 py-2 text-gray-700 bg-gray-200 rounded" id="option{{ $site_footer->id}}" name="option" rows="12">{{ $site_footer->option }}</textarea>
                        <div class="mt-6">
            ...
            <div id="" class="" x-show="openTab === 2">
                <span>{{ $site_name->option }}</span>

                <span>{{ $site_description->option }}</span>
            </div>
